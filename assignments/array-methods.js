...
// Problem 1- How many different companies are represented by our race.
let whatCompanies = runners.map(item => item.company_name);
console.log(whatCompanies);
// Problem 2 Need all the first and last names of runners with a donation greater than 100
let bigMoney = runners.filter(item => item.donation >= 100);
let biggestDonors = bigMoney.map(
  element => `${element.last_name}, ${element.first_name}`
);
console.log(biggestDonors);

// Problem 3 tally the total donations of those who bought small shirts
let smallShirts = runners.filter(item => item.shirt_size.includes("S"));
let totatlMoneySmall = smallShirts.reduce((sum, item) => {
  return (sum = +item.donation);
}, 0);
console.log(totatlMoneySmall);
